...
class SettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // Collect all non-file setting keys to skip
        $fileKeys = ['logo', 'favicon', 'home_hero_background', 'signup_brand_logo', 'signin_brand_logo'];

        // Save each setting
        foreach ($validated as $key => $value) {
            // Skip file fields and removal checkboxes
            if (in_array($key, $fileKeys) || strpos($key, 'remove_') === 0 || strpos($key, 'shipping_region_costs') === 0) {
                continue;
            }

            // Handle shipping_region_costs array as JSON
            if ($key === 'shipping_region_costs' && is_array($value)) {
        ...
        // Handle shipping_region_costs if present
